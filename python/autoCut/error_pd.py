...
class ERRORS:
    ...
    def create_structure(self, img_info, detection_result):
        ...
        """
        for category, classes in zip(detection_result, self.classes): # 遍历种类
            for index, error in enumerate(category): # 遍历预测的个数
                error = error.tolist()
                temp_dict = dict()
                ...
                logger.info(f"Error score: {error[4]}")
                # 将字典数据添加到DataFrame中
                if error[4] > ERROR_SCORE_MIN:
    ...
    def de_diff_overlap(self, errors_dict: dict, iou_score: float = 0.8, out_path="overlap_filtered_errors.json",
        ...
        for error in sorted_errors_dict:
            ...
            if ans and error["hic_loci"][0] <= temp_compare["hic_loci"][1]:
                ...
                if float(counted_score) > float(iou_score): # if the overlap larger than iou_score, treat as same error. then save the error with higher score
                    # judge which one's resolution is higher
                    if int(error["resolution"]) == int(temp_compare["resolution"]):
                        insert_result = max(error, temp_compare, key=lambda item: item["score"]) # select the error with the higher score
                    else:
                        # select the error with the highest resolution
                        insert_result = min(error, temp_compare, key=lambda item: item["resolution"]) # select the error with the higher resolution (smaller in resolution value)
                ...
                else:  # not same error but have overlap, save the larger one, deprecate the smaller one
    ...
    def chr_len_filter(self, errors_dict: dict, chr_len: int = None,
        ...
        if chr_len is None:
            logger.info("chr_len is None, skip chr_len check")
            filtered_errors = errors_dict
            for key in filter_cls:
                try:
        ...
        with open(os.path.join(self.out_path, "error_summary.json"), "a") as outfile:
            ...
            logger.info("Save \'error_summary.json\' within Function \'chr_len_filter\'")
    ...
    def loci_zoom(self, errors_dict, threshold=3000, out_path="zoomed_errors.json", filter_cls=None):
        ...
        Args:
            ...
            out_path: output path
            filter_cls: filter class, if None, then filter self.classes
...
def json_vis(error_json, out_dir):
    ...
    for key in error_dict.keys():
        for index, error in enumerate(error_dict[key]):
            ...
            if DEBUG:
                logger.info(f"Figure with error annotation generated: {basename}")
...
def infer_error(
    ...
    info_file = os.path.join(img_path, "info.json")
    infos = []
    with open(info_file, "r") as f:
        lines = ''.join(line.replace('\n', '') for line in f.readlines())
    tmp = json.loads(lines)
    infos = [{i: tmp[i]} for i in tmp]
    ...
    for item in os.listdir(img_path):
        ...
        if item_path.endswith(".png"):
            img_size = Image.open(item_path).size
    ...
    for i, info in enumerate(infos):
        detection_result = inference_detector(
            model, 
            list(info.keys())[0])
        logger.info(f"Processing: [{i}]: {list(info.keys())[0]}")

        error_class.create_structure(info, detection_result[0])
        # if DEBUG and error_class.df.size>0: break
    ...
    # score filter
    score_filtered_errors, score_filtered_errors_counter = error_class.filter_all_errors(
        score=score,
        filter_cls=classes)

    # length filter
    len_filtered_errors, len_filtered_errors_counter = error_class.len_filter(
        score_filtered_errors,
    ...
    # overlap filter
    overlap_filtered_errors, overlap_filtered_errors_counter = error_class.de_diff_overlap(
        len_filtered_errors_json,
        iou_score=iou_score)

    # chromosome length filter
    chr_filtered_errors, chr_filtered_errors_counter = error_class.chr_len_filter(
        overlap_filtered_errors,
        chr_len=chr_len)

    # zoom error
    zoom_threshold = 0
    zoomed_error = error_class.loci_zoom(
        chr_filtered_errors, 
        threshold=zoom_threshold, 
        out_path="zoomed_errors.json",
        filter_cls=None)
    ...
    error_json = os.path.join(out_path, "chr_len_filtered_errors.json" if chr_len!=None else "overlap_filtered_errors.json")
